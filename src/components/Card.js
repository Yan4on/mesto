export default class Card {
    ...
    setStateLike() {
      this._card
          .querySelector(".grid-card__like")
          .classList
          .toggle("grid-card__like_active");
    }
  
    // Установка количества лайков на странице
    setCountLikeToPage(likes) {
      this._likes = likes; // Обновляем массив лайков в объекте
      this._card.querySelector(".grid-card__likes-count").textContent = likes.length;
    }
    ...
    createCard() {
      ...
      const elementLikes = this._card.querySelector(".grid-card__likes-count");
      this._btnRemove = this._card.querySelector(".grid-card__del");
      ...
      if (this._ownerId === this._userID) {
        this._btnRemove.classList.add("grid-card__del_active")
      }
